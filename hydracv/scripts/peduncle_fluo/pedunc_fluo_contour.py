...
def pedunc_trace(file_icy, file_dlc, max_depth, video, scale = (1, 1), display=True):
    ...
    for iframe in tqdm(range(num_frames)):
        ...
        if np.isnan(markers[0]):
            try:
                intensities_.append(intensities_[-1])
                lengths.append(lengths[-1])
            except:
                intensities_.append(0)
                lengths.append(0)
            continue
...
if __name__ == '__main__':
    ...
    lengths = pedunc_trace(df.IcyFilePath.values[0],
                df.DeeplabcutFilePath.values[0],
                df.MaxDepth.values[0], df.VideoPath.values[0],
                (df.ScaleX.values[0], df.ScaleY.values[0]), display=True)
